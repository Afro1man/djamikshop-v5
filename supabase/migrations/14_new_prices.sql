-- ═══════════════════════════════════════════════════════════════════
--  Mise a jour prix : VIP 1500, PREMIUM 2500 (au lieu de 3000/5000)
-- ═══════════════════════════════════════════════════════════════════
...
begin
  ...
  -- NOUVEAUX PRIX
  amt := case p_tier when 'vip' then 1500 when 'premium' then 2500 end;
  final_amt := amt;
  admin_note_val := null;

  -- Promo si eligible (WELCOME50 = -50% sur 1er mois VIP)
  if p_promo_code is not null and p_promo_code != '' then
    if upper(p_promo_code) = 'WELCOME50' and p_tier = 'vip' and public.is_promo_eligible(uid, 'WELCOME50') then
      final_amt := amt / 2;   -- 750 FCFA pour le 1er mois VIP avec promo
